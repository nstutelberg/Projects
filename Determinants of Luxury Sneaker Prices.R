...
AIC2 <- lm(`Log_Sale_Price` ~ `Shoe_Size_Squared` + `Log_Days_Since_IPO` + `Interaction_Color_Stripe`, data = shoestotal)
    ...
    #no shoe size/squared + break out interaction 
AIC3 <- lm(`Log_Sale_Price` ~ `Log_Days_Since_IPO` + `Light_Dark` +`Solid_Stripe`, data = shoestotal)

    #no shoe squared
AIC4 <- lm(`Log_Sale_Price` ~ `Shoe Size` + `Log_Days_Since_IPO` + `Interaction_Color_Stripe`, data = shoestotal) 

    #no log ipo
AIC5 <- lm(`Log_Sale_Price` ~ `Shoe Size` + `Shoe_Size_Squared` + `Interaction_Color_Stripe`, data = shoestotal)

    #no interaction
AIC6 <- lm(`Log_Sale_Price` ~ `Shoe Size` + `Shoe_Size_Squared` + `Log_Days_Since_IPO`, data = shoestotal)

models <- list(AIC1, AIC2, AIC3, AIC4, AIC5, AIC6)
aictab(cand.set = models, modnames = c('All Variables', 'No Shoe Size', 'No Shoe Size/Squared + Break out Interaction ', 'No Shoe Size Squared', 'No Log Days Since IPO', 'No Color Stripe Interaction'))
